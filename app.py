...
h1, h2, h3 {{ color: #1a202c; font-weight: 700; }}
                ...
                if "languages" in df.columns:
                    with st.expander("Top Programming Languages Used"):
                        ...
                        if lang_series.empty:
                            ...
                            fig, ax = plt.subplots(figsize=(8, 5))
                            lang_series.sort_values().plot(kind="barh", ax=ax, color="#10b981")
                            ax.set_xlabel("Count")
...
if not st.session_state.logged_in:
    ...
    if _current not in _auth_options:
        _current = "User Login"
    auth_choice = st.sidebar.radio("Select Option", _auth_options,
...
if not st.session_state.logged_in:
    ...
    st.markdown("""
        ...
        .auth-form-logo { font-family:'DM Sans',sans-serif; font-size:1rem; font-weight:700; color:#667eea; margin-bottom:0.3rem; }
        .auth-form-title { font-family:'Playfair Display',serif; font-size:1.7rem; font-weight:700; color:#1a202c; margin-bottom:0.3rem; white-space:nowrap; }
        .auth-form-subtitle { font-size:0.82rem; color:#9ca3af; margin-bottom:1.5rem; }
        .auth-form-divider { border:none; border-top:1px solid #f0f0f0; margin:1rem 0 1.5rem; }
        .auth-hero-panel {
            border-radius:24px;
        ...
        .auth-hero-sub { color:rgba(255,255,255,0.7); font-size:0.9rem; line-height:1.7; margin-bottom:2rem; }
